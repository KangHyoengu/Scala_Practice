...
object LearnSet {
  /*
  * Set은 Set(element1, element2, ...)과 같이 생성합니다.
  * 스칼라에서 기본 Set은 Predef.Set입니다.
  * Set은 크기가 4일때까지는 크기에 따라 별도 클래스가 있습니다. Set1, Set2, Set3, Set4 구성요소가 많아지면 HashSet으로 구현됩니다.
  * 집합에 대응하는개념으로 같은 값을 추가하면 기존값을 덮어쓰게 되고, 순서가 보장되지 않습니다.
  * */

  def main(args: Array[String]): Unit = {
    // 1. 내용을 수정할 수 없는 Set
    val set1 = Set("one", 1)
    val set2 = Set(1, 2, 2, 2, 3, 3, 3) // 중복이 제거되고 Set(1, 2, 3)이 된다.
    println(s"1. $set1, $set2")

    // 2. 값이 있는지 체크하는 방법은 괄호 안에 값을 넣어서 사용한다.
    val oneExists = set2(1)
    val fourExists = set2(4)
    println(s"2. oneExists: ${oneExists}, fourExists: ${fourExists}")

    // 3. Set을 더하면 중복된 내용은 제거된 새로운 Set이 생성된다.
    ...
    /*
     * 5. set.find 메서드를 이용해서 findByName이라는 메서드 생성
     * find는 조건에 맞는 값을 찾으면 검색을 중단
     * getOrElse는 일치하는 값이 없을 경우 넘겨줄 기본값
